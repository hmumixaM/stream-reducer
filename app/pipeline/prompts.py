...
MAP_SYSTEM = (
    ...
    "The transcript is auto-generated by speech-to-text and may contain recognition "
    "errors — misheard homophones, garbled or inconsistent proper nouns (people, "
    "companies, products, places), wrong technical terms, and mistyped numbers. Use "
    "the page title/description provided and your general knowledge to SILENTLY "
    "correct such obvious transcription errors so names and terms are spelled "
    "correctly and used consistently throughout. Correct only clear mis-transcriptions "
    "without changing the speaker's actual meaning; never invent facts, and when you "
    "are unsure, keep the original wording."
)

MAP_TEMPLATE = """## Page background (title, channel, and description — use it to fix transcription errors)
{context}

Below is part {index} of {total} of a transcript. Each line is
prefixed with its [HH:MM:SS] timestamp. It is auto-generated (speech-to-text), so
expect recognition errors.

Write a detailed, faithful walkthrough of THIS part as Markdown:
- The transcript may mis-hear words. Using the page background above and common
  knowledge, silently correct obvious transcription errors — especially names,
  companies, products, places, and technical terms — so they are correct and
  consistent. Do NOT change the actual meaning or invent content; when unsure,
  keep the original.
...
- When a visual architecture or flow is important, use a fenced Mermaid diagram
  ...
  plain code fence. For genuinely tabular comparisons, use a standard Markdown
  table instead. If neither format can express the source cleanly, explain it
  in prose.
...
INFOGRAPHIC_SYSTEM = (
    ...
    "Use that space for information instead.\n"
    "- Maximize useful information density while keeping every label clearly legible. "
    "Include as many distinct, accurate facts from the supplied content as the layout permits.\n"
